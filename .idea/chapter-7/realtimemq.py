...
def recognize():
    ...
    img_dataset = home+'/IdeaProject/dataset/'

    # 2. Path for haar-cascade pretrained model
    haar_cascade_path = home+'/IdeaProject/haarcascade_frontalface_default.xml'
    face_detection = cv2.CascadeClassifier(haar_cascade_path)
    ...
    # 4. Trained model path
    trained_model = home+'/IdeaProject/trainer.yml'

    # 4. Path of employee database
    database = pd.read_csv(home+'/IdeaProject/face_database.csv')
    min_acc = 60
